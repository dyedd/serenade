<template>
    <ul class="mt-8 flex flex-row items-center">
        <li v-if="currentPage > 1" class="mx-1 min-w-[1.8rem] text-center">
            <a @click="goToPage(currentPage - 1)" class="block rounded hover:bg-primary-600 hover:text-neutral"
                aria-label="Previous page" rel="prev">
                ←
            </a>
        </li>
        <li v-for="page in visiblePages" :key="page" class="mx-1 min-w-[1.8rem] text-center">
            <span v-if="page === currentPage" aria-current="page" aria-label="Page {{ page }}"
                class="block rounded bg-primary-200 font-semibold text-primary-700 dark:bg-primary-400 dark:text-neutral-800">
                {{ page }}
            </span>
            <a v-else @click="goToPage(page)" class="block rounded hover:bg-primary-600 hover:text-neutral"
